...
template <typename traits> class BSkipNodeLeaf : public BSkipNode<traits> {
public:
...
private:
  uint32_t find_index_linear(K k) {
    ...
    for (i = 0; i < BSkipNode<traits>::num_elts; i++) {
      ...
      if (blind_read_key(i) < k)
        continue;
      else if (k == blind_read_key(i)) { // (k == keys[i]) {
  ...
  uint32_t find_index_binary(K k) {
    ...
    while (left <= right) {
      ...
      if (blind_read_key(mid) == k) { // (keys[mid] == k) {
        left = mid;
        break;
      } else if (blind_read_key(mid) < k) { // (keys[mid] < k) {
        left = mid + 1;
      } else {
        if (mid == 0) {
          break;
...
// extra case for internal node
template <typename traits> class BSkipNodeInternal : public BSkipNode<traits> {
  using K = traits::key_type;
...
public:
  ...
  void set_key_at_rank(uint32_t rank, K key) { keys[rank] = key; }

  // add child pointer
  void insert_child_at_rank(uint32_t rank, BSkipNode<traits> *elt) {
...
// bskip structure
template <typename traits> class BSkip {
  using K = traits::key_type;
...
public:
  ...
  BSkipNode<traits> *find(traits::key_type k);
  bool exists(traits::key_type k);

	template <class F> void map_range_length(traits::key_type start, uint64_t length, F f) const;
	template <class F> void map_range(traits::key_type min, traits::key_type max, F f) const;
  ...
  BSkip() {
    prob = traits::p;
    promotion_probability = (double)(1.0) / (double)prob;
    ...
    for (uint32_t i = MAX_HEIGHT - 1; i >= 0; i--) {
      headers[i]->num_elts++;
      if (i > 0) {
...
template <typename traits> uint32_t BSkip<traits>::flip_coins(K k) {
  uint32_t result = 0;
  size_t h = std::hash<K>{}(k);
  uint64_t flip = h % prob;
  while (flip == 0) {
    result++;
...
bool BSkip<traits>::insert(traits::element_type k) {
  ...
  std::vector<BSkipNode<traits> *> new_nodes;
...
#endif
  for (uint32_t level = MAX_HEIGHT - 1; level >= 0; level--) {
#if DEBUG_PRINT
    if (level > 0) {
    ...
    while (curr_node->next->get_header() <= key) {
    ...
    uint32_t rank = curr_node->find_key(key);

    // if the key was found
    if (curr_node->get_key_at_rank(rank) == key) {
...
#if WEIGHTED
      // TODO: in weighted mode, you have to grab writer locks all the way down
      // and update the value
      // TODO: this is currently wrong bc internal nodes don't have values, fix
      // it later if the key is there, update values all the way down (in the
      // weighted case)
      level--;
      if (level > 0) {
        ...
        while (level >= 0) {
          ...
          if (level > 0) {
            ...
            return true; // exit if we are at the last level
          }
          level--;
        }
      }
...
template <typename traits>
BSkipNode<traits> *BSkip<traits>::find(traits::key_type k) {
  ...
  for (int level = MAX_HEIGHT - 1; level >= 0; level--) {
    ...
    while (curr_node->next->get_header() <= k) {
#if DEBUG_PRINT
...
void BSkip<traits>::map_range(traits::key_type min, traits::key_type max, F f) const {
  ...
  for(int level = MAX_HEIGHT-1; level >= 0; level--) {
    ...
    while(current->next->get_header() <= min) {
    ...
    if(current->get_key_at_rank(current_key_rank) == min) {
      found = true;
      break;
    } else if(level != 0) {
      if constexpr(traits::concurrent) {
...
void BSkip<traits>::map_range_length(traits::key_type start, uint64_t length, F f) const {
  ...
  for(int level = MAX_HEIGHT-1; level >= 0; level--) {
    ...
    if(current->get_key_at_rank(current_key_rank) == start) {
      found = true;
      break;
    } else if(level != 0) {
      if constexpr(traits::concurrent) {
...
// true if found
template <typename traits> bool BSkip<traits>::exists(K k) {
  auto node = find(k);
  return node ? true : false;
}

// sums all keys
template <typename traits> uint64_t BSkip<traits>::sum() {
...
// sums all keys
template <typename traits> uint64_t BSkip<traits>::sum_vals() {
  auto curr_node = headers[0];
  uint64_t result = 0;
  while (curr_node->get_header() < std::numeric_limits<K>::max()) {
...
// sums all keys
template <typename traits> uint64_t BSkip<traits>::psum() {
...
template <typename traits> void BSkip<traits>::get_size_stats() {
  ...
  for (int level = MAX_HEIGHT - 1; level >= 0; level--) {
    ...
    double density = (double)elts_per_level[level] /
                     (double)(nodes_per_level[level] * traits::max_size);
...
template <typename traits> void BSkip<traits>::get_avg_comparisons() {
...
template <typename traits> void BSkip<traits>::validate_structure() {
  for (int level = MAX_HEIGHT - 1; level >= 0; level--) {
    auto curr_node = headers[level];
