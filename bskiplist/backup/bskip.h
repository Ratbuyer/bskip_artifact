    ...
    void delete_key(traits::key_type k)
{
    ...
    if (k == 0 || k == std::numeric_limits<K>::max()) {
        return;
    }
    ...
    // Phase 1: Find all nodes containing k and collect them
    // We'll store (level, parent_node, parent_rank, node_with_k) tuples
    struct DeletionInfo {
        int level;
        BSkipNode<traits> *parent;  // Node whose child pointer needs updating
        uint32_t parent_rank;       // Rank in parent where child pointer is
        BSkipNode<traits> *node;    // Node containing k (to be deleted or modified)
        uint32_t rank_in_node;      // Rank of k within node
        bool is_header;             // Is k the header of this node?
    };

    std::vector<DeletionInfo> deletion_path;
    deletion_path.reserve(MAX_HEIGHT);

    // Start search from the top
    auto curr_node = headers[MAX_HEIGHT - 1];
    BSkipNode<traits> *parent_for_next_level = nullptr;
    uint32_t parent_rank_for_next_level = 0;

    // Search down through levels
    for (int level = MAX_HEIGHT - 1; level >= 0; level--) {
        ...
        // Move right to find the correct node
        while (curr_node->next_header <= k) {
            auto next_node = curr_node->next;

            if constexpr (traits::concurrent) {
                if (level > 0) {
                    ((BSkipNodeInternal<traits> *)next_node)->mutex_.read_lock(cpuid);
                    ((BSkipNodeInternal<traits> *)curr_node)->mutex_.read_unlock(cpuid);
                } else {
                    ((BSkipNodeLeaf<traits> *)next_node)->mutex_.read_lock(cpuid);
        ...
        if (found && curr_node->get_key_at_rank(rank) == k) {
            // Found the key at this level
            DeletionInfo info;
            info.level = level;
            info.parent = parent_for_next_level;
            info.parent_rank = parent_rank_for_next_level;
            info.node = curr_node;
            info.rank_in_node = rank;
            info.is_header = (rank == 0);

            deletion_path.push_back(info);
        }

        // Prepare for next level
        if (level > 0) {
            parent_for_next_level = curr_node;
            parent_rank_for_next_level = rank;

            auto next_level_node = ((BSkipNodeInternal<traits> *)curr_node)->get_child_at_rank(rank);

            if constexpr (traits::concurrent) {
                ((BSkipNodeInternal<traits> *)curr_node)->mutex_.read_unlock(cpuid);
            }

            curr_node = next_level_node;
        } else {
            if constexpr (traits::concurrent) {
                ((BSkipNodeLeaf<traits> *)curr_node)->mutex_.read_unlock(cpuid);
            }
    ...
    // If key not found at any level, return
    if (deletion_path.empty()) {
#if DEBUG_PRINT
        printf("key %lu not found for deletion\n", k);
#endif
        return;
    }

    // Phase 2: Delete from bottom to top
    // This ensures structural consistency

    for (int i = 0; i < deletion_path.size(); i++) {
        auto &info = deletion_path[i];
        int level = info.level;

        if (!info.is_header) {
            // Simple case: k is not the header, just remove it from the node
            if constexpr (traits::concurrent) {
                if (level > 0) {
                    ((BSkipNodeInternal<traits> *)info.node)->mutex_.write_lock();
                } else {
                    ((BSkipNodeLeaf<traits> *)info.node)->mutex_.write_lock();
                }
            }

            // Re-validate that k is still there
            auto [new_rank, still_found] = info.node->find_key_and_check(k);
            if (!still_found || info.node->get_key_at_rank(new_rank) != k) {
                // Key already deleted by another thread
                if constexpr (traits::concurrent) {
                    if (level > 0) {
                        ((BSkipNodeInternal<traits> *)info.node)->mutex_.write_unlock();
                    } else {
                        ((BSkipNodeLeaf<traits> *)info.node)->mutex_.write_unlock();
                    }
                }
                continue;
            }

            // Delete the key and child pointer (for internal nodes)
            if (level > 0) {
                ((BSkipNodeInternal<traits> *)info.node)->delete_key_at_rank(new_rank);
                ((BSkipNodeInternal<traits> *)info.node)->delete_child_at_rank(new_rank);
            } else {
                ((BSkipNodeLeaf<traits> *)info.node)->delete_key_at_rank(new_rank);
            }
            info.node->num_elts--;
            ...
            // Complex case: k is the header of the node
            // Need to find left neighbor and update pointers

            BSkipNode<traits> *left = headers[level];
            BSkipNode<traits> *target = info.node;

            // Acquire locks in order: left first, then target
            if constexpr (traits::concurrent) {
                if (level > 0) {
                    ((BSkipNodeInternal<traits> *)left)->mutex_.write_lock();
                } else {
                    ((BSkipNodeLeaf<traits> *)left)->mutex_.write_lock();
                }
            }

            // Find the node whose next pointer points to target
            while (left->next != target) {
                auto next = left->next;

                if constexpr (traits::concurrent) {
                    if (level > 0) {
                        ((BSkipNodeInternal<traits> *)next)->mutex_.write_lock();
                        ((BSkipNodeInternal<traits> *)left)->mutex_.write_unlock();
                    } else {
                        ((BSkipNodeLeaf<traits> *)next)->mutex_.write_lock();
                        ((BSkipNodeLeaf<traits> *)left)->mutex_.write_unlock();
                    }
                }
            ...
            if constexpr (traits::concurrent) {
                if (level > 0) {
                    ((BSkipNodeInternal<traits> *)target)->mutex_.write_lock();
                } else {
                    ((BSkipNodeLeaf<traits> *)target)->mutex_.write_lock();
                }
            }

            // Re-validate that k is still the header
            if (target->num_elts == 0 || target->get_key_at_rank(0) != k) {
                // Already deleted
                if constexpr (traits::concurrent) {
                    if (level > 0) {
                        ((BSkipNodeInternal<traits> *)left)->mutex_.write_unlock();
                        ((BSkipNodeInternal<traits> *)target)->mutex_.write_unlock();
                    } else {
                        ((BSkipNodeLeaf<traits> *)left)->mutex_.write_unlock();
                        ((BSkipNodeLeaf<traits> *)target)->mutex_.write_unlock();
                    }
                }
                continue;
            }

            if (target->num_elts == 1) {
                // Node becomes empty, remove it entirely
                auto next_node = target->next;
                K next_header = next_node->get_header();

                left->next = next_node;
                left->next_header = next_header;
                ...
                // Node still has elements, just remove the header
                if (level > 0) {
                    ((BSkipNodeInternal<traits> *)target)->delete_key_at_rank(0);
                    ((BSkipNodeInternal<traits> *)target)->delete_child_at_rank(0);
                } else {
                    ((BSkipNodeLeaf<traits> *)target)->delete_key_at_rank(0);
                }
                target->num_elts--;

                // Update left's next_header
                K new_header = target->get_key_at_rank(0);
                left->next_header = new_header;
