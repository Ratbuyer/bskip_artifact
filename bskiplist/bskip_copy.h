...
public:
    ...
    virtual traits::key_type get_max_key() = 0;

    virtual void update_max_key() = 0;
...
public:
    ...
    void blind_write(traits::element_type e, uint32_t index)
    {
        ...
        if (traits::get_key(e) > this->max_key)
        {
            this->max_key = traits::get_key(e);
        }
    }
    ...
    inline K get_max_key()
    {
        return blind_read_key(this->num_elts - 1); // return keys[num_elts - 1];
    }

    inline void update_max_key()
    {
        this->max_key = blind_read_key(this->num_elts - 1);
    }
...
public:
    ...
    inline K get_max_key() { return keys[this->num_elts - 1]; }

    inline void update_max_key()
    {
        this->max_key = keys[this->num_elts - 1];
    }
...
public:
    ...
    mutable std::atomic<int> root_lock_counter = 0;
#endif
    ...
    ~BSkip()
    {
        ...
        printf("range length nodes searched: %d\n", range_length_nodes);
        
        int root_locks = root_lock_counter.load();
        printf("root locks aquired = %d\n", root_locks);
#endif
...
#if STATS
	if (level_to_promote == MAX_HEIGHT - 1) {
		root_lock_counter++;
	}
#endif
...
// stats for how many steps horizontally taken
    ...
    for (int level = MAX_HEIGHT - 1; level >= 0; level--)
    {
        ...
        while (k > curr_node->max_key && k >= curr_node->next->get_header())
        {
#if DEBUG_PRINT
            printf("\tcurr node header %lu\n", curr_node->get_header());
            printf("\tnext node header %lu\n", curr_node->next->get_header());
...
void BSkip<traits>::map_range_length(traits::key_type start, uint64_t length, F f) const
{
    ...
    for (int level = MAX_HEIGHT - 1; level >= 0; level--)
    {
        ...
        // Search through each level until you find the correct node range
        // Is get header able to be called on a node that does not have a read lock
        while (start > current->max_key && start >= current->next->get_header())
...
void BSkip<traits>::validate_structure()
{
    ...
    for (int level = MAX_HEIGHT - 1; level >= 0; level--)
    {
        ...
        while (curr_node->get_header() < std::numeric_limits<K>::max())
        {
            ...
            {
                if (curr_node->get_max_key() != curr_node->max_key)
                    violations++;
            }
